...
| 内容 | 大小 | 来源 | 时机 |
|---|---|---|---|
| ffmpeg + ffprobe 便携版 | ~90 MB | gyan.dev（ffmpeg 官方推荐 Windows 构建） | `setup.py --install` |
| yt-dlp.exe 便携版 | ~15 MB | GitHub yt-dlp 官方 releases | `setup.py --install` |
| Python 包（faster-whisper 等） | ~100 MB | PyPI | `setup.py --install` |
| CUDA 库（可选，N 卡加速） | ~1 GB | PyPI | 加 `--with-cuda` 时 |
...
|---|---|---|---|
| ffmpeg + ffprobe (portable) | ~90 MB | gyan.dev (officially recommended Windows build) | `setup.py --install` |
| yt-dlp.exe (portable) | ~15 MB | Official yt-dlp GitHub releases | `setup.py --install` |
| Python packages (faster-whisper etc.) | ~100 MB | PyPI | `setup.py --install` |
| CUDA libraries (optional, NVIDIA GPU boost) | ~1 GB | PyPI | with `--with-cuda` |
